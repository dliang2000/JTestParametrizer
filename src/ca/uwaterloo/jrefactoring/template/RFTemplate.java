...
public class RFTemplate {
    ...
    */
    ...
    private void saveMethod(ICompilationUnit cu, MethodDeclaration method, List<Name> imports, boolean replace) throws Exception {
        ...
        // check if IgnoreClone.java has already been created for the benchmark project - if not, create it
        IPath javaDirPath = astRoot.getJavaElement().getResource().getRawLocation();
        while(!javaDirPath.lastSegment().equals("java")) {
        	javaDirPath = javaDirPath.removeLastSegments(1);
        }
        IPath ignoreAnnotationPath = javaDirPath.addTrailingSeparator().append("ca/uwaterloo/jrefactoring/autogenerated/IgnoreClone.java");
        if(!ignoreAnnotationPath.toFile().exists()) {
        	ignoreAnnotationPath.removeLastSegments(1).toFile().mkdirs();
        	PrintWriter writer = new PrintWriter(ignoreAnnotationPath.toString(), "UTF-8");
        	writer.println("package ca.uwaterloo.jrefactoring.autogenerated;");
        	writer.println("public @interface IgnoreClone {}");
        	writer.close();
        }
